...
#define FIND_INDEX_EXISTS_LINEAR(n, nIndex, fi, li, in)		\
    if (Traits::randomAccessType == USE_LINEAR_SEARCH)         \
    {                                                           \
	in = fi+1;						\
	while (n > nIndex[in])					\
...
void DTGrid<Traits>::closestPoint(const Index p[3], Real n[3], Real *cd, bool *inside, bool *inNarrowBand) const
{
...
template<class Traits>
void DTGrid<Traits>::computeGradient(Real n[3], Real v[6]) const
{
    n[0] = (v[1] - v[0]) / ( 2*dx );
    n[1] = (v[3] - v[2]) / ( 2*dx );
    n[2] = (v[5] - v[4]) / ( 2*dx );
